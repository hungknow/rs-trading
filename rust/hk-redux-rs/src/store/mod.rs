...
use crate::{
    middleware::{MiddleWare, StoreApi, StoreWithMiddleware}, subscriber::SubscriberAsync, Reducer, Selector, Subscriber
};
    ...
    State: Send + 'static,
{
    ...
    // pub async fn subscribe_async<S: SubscriberAsync<State> + Send + 'static>(&self, subscriber: S) {
    //     self.worker_address.send(SubscribeAsync::new(Box::new(subscriber))).await;
    // }
