    ...
    RootReducer: Send,
{
    ...
    // subscribers_async: Vec<Box<dyn SubscriberAsync<State> + Send>>,
    // state_change_senders: mpsc::Sender<Arc<Box<State>>>,
    // state_change_sender: tokio::sync::watch::Receiver<Arc<Box<State>>>,
}
    ...
    RootReducer: Send,
{
    pub fn new(root_reducer: RootReducer, state: State) -> Self {
        Self {
            ...
            // subscribers_async: Default::default(),
            // state_change_senders: Default::default(),
        }
    }
    ...
    Action: Send,
{
    async fn handle_work(&mut self, work: Dispatch<Action>) {
        ...
        // if !self.subscribers_async.is_empty() {
        //     let new_state = self.state.as_ref().unwrap();
        //     for subscriber in &self.subscribers_async {
        //         // subscriber.notify(new_state).await;
        //     }
        // }

        // if !self.state_change_senders.is_empty() {
        //     let new_state = Arc::new(self.state.unwrap());
        //     for sender in &self.state_change_senders {
...
// #[async_trait]
// impl<State, Action, RootReducer> HandleWork<SubscribeAsync<State>>
//     for StateWorker<State, Action, RootReducer>
// where
//     RootReducer: Reducer<State, Action>,
//     State: Send,
//     RootReducer: Send,
// {
//     async fn handle_work(&mut self, work: SubscribeAsync<State>) {
//         let subscriber = work.into_subscriber();
//         self.subscribers_async.push(subscriber);
//     }
// }
